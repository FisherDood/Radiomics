...
for i = 1:length(names)
    ...
    if strcmp(image.classUID,'') 
        ...
        if isfield(info, 'AcquisitionDate')
            image.acquisitionDate = info.AcquisitionDate;
        end
        image.width(3) = info.SliceThickness / 10; % cm
        %image.width(3) = info.SpacingBetweenSlices / 10; % cm
    ...
    %elseif image.width(3) ~= info.SpacingBetweenSlices / 10
        if exist('Event', 'file') == 2
...
info.ImageOrientationPatient=round(info.ImageOrientationPatient);
if isequal(info.ImageOrientationPatient, [1;0;0;0;1;0]) || ...
        isequal(info.ImageOrientationPatient, [-1;0;0;0;-1;0]) 
